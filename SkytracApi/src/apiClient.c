...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://192.168.20.1/api");
    ...
    apiClient->apiKeys_QueryAuthAllRoles = NULL;
...
, list_t *apiKeys_QueryAuthAllRoles
) {
    apiClient_t *apiClient = malloc(sizeof(apiClient_t));
    if(basePath){
    ...
    apiClient->username = NULL;
    apiClient->password = NULL;
    if(apiKeys_QueryAuthAllRoles!= NULL) {
        apiClient->apiKeys_QueryAuthAllRoles = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_QueryAuthAllRoles) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_QueryAuthAllRoles, pairDup);
        }
    }else{
        apiClient->apiKeys_QueryAuthAllRoles = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_QueryAuthAllRoles) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_QueryAuthAllRoles) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_QueryAuthAllRoles);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        if (apiClient->apiKeys_QueryAuthAllRoles != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_QueryAuthAllRoles) {
